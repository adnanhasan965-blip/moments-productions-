...
/** On-demand branded PDF of a project's schedule (session-authed). */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const pdf = await renderUrlToPdf(
      `${origin}/print/schedule/${projectId}?key=${project.share_key}&lang=${lang}`
    );
    const safeName = project.name.replace(/[^\w-]+/g, "_");
    return new NextResponse(Buffer.from(pdf), {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="${safeName}_schedule.pdf"`,
      },
    });
  } catch (e) {
