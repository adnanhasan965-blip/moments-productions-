...
/** On-demand project booklet PDF: overview + schedule + every day's
 *  call sheet and shot list (session-authed). */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const pdf = await renderUrlToPdf(
      `${origin}/print/booklet/${projectId}?key=${project.share_key}&lang=${lang}`,
    ...
    return new NextResponse(Buffer.from(pdf), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${safeName}_booklet.pdf"`,
      },
    });
  } catch (e) {
